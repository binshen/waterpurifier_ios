...
@interface RegistPage ()
{
    ...
    UITextField * userSmailTextField;
...
- (void)viewDidLoad {
   ...
   self.title = @"注册";
    ...
    userSmailTextField = [[UITextField alloc] initWithFrame:CGRectMake(30, 220, SCREEN_WIDTH-60, 35)];
    userSmailTextField.secureTextEntry = NO;
    userSmailTextField.placeholder = @"    请填写邮箱用于接收统计报表";
    userSmailTextField.font = [UIFont fontWithName:@"Arial" size:16];
    userSmailTextField.delegate = self;
    userSmailTextField.layer.borderColor = [RgbColor(213, 213, 213) CGColor];
    userSmailTextField.layer.masksToBounds = YES;
    userSmailTextField.layer.cornerRadius = 8.0f;
    userSmailTextField.layer.borderWidth = 1.0f;
    //serSmailTextField.keyboardType = UIKeyboardTypeNumberPad;
    userSmailTextField.contentVerticalAlignment = UIControlContentVerticalAlignmentCenter;
    [self.view addSubview:userSmailTextField];
    
    resetUserPwdBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    resetUserPwdBtn.frame = CGRectMake(30, 280, SCREEN_WIDTH-60, 40);
    [resetUserPwdBtn setTitle:@"注册" forState:UIControlStateNormal];
...
-(void)registAction
{
    ...
    else if(userSmailTextField.text.length == 0)
    {
        [Global alertMessageEx:@"请输入邮箱." title:@"错误信息" okTtitle:nil cancelTitle:@"OK" delegate:self];
    }
    else if(userSmailTextField.text.length > 0 && [StringUtil isValidateEmail:userSmailTextField.text] == NO)
    {
        // 检测手机号是否合法
    ...
    else
    {
        ...
        [param setValue:userSmailTextField.text forKey:@"email"];
...
-(void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    [self textFieldShouldReturn:userSmailTextField];
}
